...
def log_sub_exp(a: Tensor, b: Tensor) -> Tensor:
    m = torch.maximum(a, b)
    return torch.log(torch.exp(a - m) - torch.exp(b - m)) + m # original return
    # return torch.log(torch.exp(torch.clamp(a - m, min=0)) - torch.exp(b - m) + 1e-10) + m # adjusted to prevent log(0) and a-m < 0 in case of rounding errors leading to b-m = 0 and a-m < 0
